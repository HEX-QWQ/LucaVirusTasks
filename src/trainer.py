...
def train(args, train_dataloader, model_config, model, seq_tokenizer, parse_row_func, batch_data_func, train_sampler=None, log_fp=None):
    ...
    if args.local_rank in [-1, 0]:
        print("Train dataset len: %d, batch size: %d, batch num: %d" % (
    ...
    if args.local_rank in [0, -1]:
        ...
        log_fp.write("Evaluating Strategy = %s\n" % args.evaluate_strategy)
        logger.info("Evaluating Strategy = %s" % args.evaluate_strategy)
        if args.evaluate_strategy == "step":
            log_fp.write("Evaluating Steps = %d steps\n" % args.evaluate_steps)
            logger.info("Evaluating Steps = %d steps" % args.evaluate_steps)
    ...
    if args.local_rank in [0, -1]:
        ...
        if args.early_stop_num_epoch is not None and args.early_stop_num_epoch > 1:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.local_rank in [0, -1]:
                ...
                if global_step % args.gradient_accumulation_steps == 0:
                    print("\rTraining, Epoch: %04d, Batch: %06d, S-N: %d, C-L: %.08f, C-A-L: %.08f, G-A-L: %.08f" % (
                        epoch + 1,
                        cur_epoch_step,
                        done_sample_num,
                        cur_loss,
            ...
            if args.local_rank in [-1, 0] and \
                ...
                args.evaluate_strategy == "step" and \
                global_step % args.evaluate_steps == 0 and args.evaluate_steps > 0:
                ...
                for key, value in eval_result.items():
                    ...
                    if key == best_metric_type:
                        if best_metric_flag and best_metric_value < value or \
                            ...
                            update_flag = True
                        # judge early stop
                        if args.early_stop_begin_epoch is not None and epoch + 1 >= args.early_stop_begin_epoch \
                                and args.early_stop_num_epoch is not None and args.early_stop_num_epoch > 1:
                            last_history_metrics.append(value)
                            if len(last_history_metrics) == args.early_stop_num_epoch:
                                if best_metric_flag:
