...
try:
    ...
    from .utils import set_seed, save_labels, get_parameter_number, get_labels, load_trained_model, print_model_structure
    ...
    from .lucatriple.models.LucaIntraInterTriple import LucaIntraInterTriple
    ...
    from src.utils import set_seed, save_labels, get_parameter_number, get_labels, load_trained_model, print_model_structure
    ...
    from src.lucatriple.models.LucaIntraInterTriple import LucaIntraInterTriple
...
def get_args():
    ...
    parser.add_argument("--dataset_type", default="protein", type=str, required=True, choices=[
        ...
        "gene_protein_gene",
        "protein_gene_gene",
        "gene_protein_protein",
        "protein_gene_protein",
        "protein_protein_gene",
        "protein_protein_protein",
    ],
                        help="dataset type")
    ...
    parser.add_argument("--seq_max_length_c", default=None, type=int,
                        help="the length of input c sequence more than max length will be truncated, shorter will be padded.")
    parser.add_argument("--matrix_max_length_c", default=None, type=int,
                        help="the length of input c embedding more than max length will be truncated, shorter will be padded.")
    parser.add_argument("--embedding_input_size_c", default=None, type=int,
                        help="b embedding_input_size")
    ...
    parser.add_argument("--not_save_emb_to_disk", action="store_true",
                        help="if this option is flagged on, then the embeddings will not be saved to disk.")
    parser.add_argument("--evaluate_strategy", default="epoch", choices=["epoch", "step"],
                        help="whether to evaluate by epoch or step.")
    parser.add_argument("--evaluate_steps", type=int, default=-1,
                        help="evaluate steps.")
    parser.add_argument("--fp16_embedding", action="store_true",
                        help="whether to use fp16 for embedding.")

    # 早停
    parser.add_argument('--early_stop_begin_epoch', default=None, type=int,
                        help='early stop begin epoch.')
    parser.add_argument('--early_stop_num_epoch', default=None, type=int,
...
def get_input_cols(args):
    ...
    elif args.input_mode == "triple" and args.input_type == "seq":
        input_col_names = [args.dataset_type, "seq_a", "seq_b", "seq_c"]
    elif args.input_mode == "triple" and args.input_type == "vector":
        input_col_names = [args.dataset_type, "embedding_vector_a", "embedding_vector_b", "embedding_vector_c"]
    elif args.input_mode == "triple" and args.input_type == "matrix":
        input_col_names = [args.dataset_type, "embedding_matrix_a", "embedding_matrix_b", "embedding_matrix_c"]
    elif args.input_mode == "triple" and args.input_type == "seq_matrix":
        input_col_names = [args.dataset_type, "seq_a", "seq_b", "seq_c", "embedding_matrix_a", "embedding_matrix_b", "embedding_matrix_c"]
    elif args.input_mode == "triple" and args.input_type == "seq_vector":
        input_col_names = [args.dataset_type, "seq_a", "seq_b", "seq_c", "embedding_vector_a", "embedding_vector_b", "embedding_vector_c"]
    else:
...
def get_model(args):
    ...
    elif args.input_mode in ["triple"]:
        assert args.seq_max_length is not None or (
                args.seq_max_length_a is not None and args.seq_max_length_b is not None and args.seq_max_length_c is not None
        )
        model_config.seq_max_length = args.seq_max_length
        if args.seq_max_length_a is not None and args.seq_max_length_b is not None and args.seq_max_length_c is not None:
            model_config.seq_max_length_a = args.seq_max_length_a
            model_config.seq_max_length_b = args.seq_max_length_b
            model_config.seq_max_length_c = args.seq_max_length_c
        else:
            model_config.seq_max_length_a = args.seq_max_length
            model_config.seq_max_length_b = args.seq_max_length
            model_config.seq_max_length_c = args.seq_max_length

        model_config.seq_max_length = max(
            model_config.seq_max_length_a, model_config.seq_max_length_b, model_config.seq_max_length_c
        )
        args.seq_max_length = model_config.seq_max_length
        assert args.matrix_max_length is not None or (
                args.matrix_max_length_a is not None and args.matrix_max_length_b is not None and args.matrix_max_length_c is not None
        )
        model_config.matrix_max_length = args.matrix_max_length
        if args.matrix_max_length_a is not None and args.matrix_max_length_b is not None and args.matrix_max_length_c is not None:
            model_config.matrix_max_length_a = args.matrix_max_length_a
            model_config.matrix_max_length_b = args.matrix_max_length_b
            model_config.matrix_max_length_c = args.matrix_max_length_c
        else:
            model_config.matrix_max_length_a = args.matrix_max_length
            model_config.matrix_max_length_b = args.matrix_max_length
            model_config.matrix_max_length_c = args.matrix_max_length
        model_config.matrix_max_length = max(
            model_config.matrix_max_length_a, model_config.matrix_max_length_b, model_config.matrix_max_length_c
        )
        args.matrix_max_length = model_config.matrix_max_length
        assert args.embedding_input_size is not None or (
                args.embedding_input_size_a is not None and args.embedding_input_size_b is not None and args.embedding_input_size_c is not None
        )
        model_config.embedding_input_size = args.embedding_input_size
        if args.embedding_input_size_a is not None and args.embedding_input_size_b is not None and args.embedding_input_size_c is not None:
            model_config.embedding_input_size_a = args.embedding_input_size_a
            model_config.embedding_input_size_b = args.embedding_input_size_b
            model_config.embedding_input_size_c = args.embedding_input_size_c
        else:
            model_config.embedding_input_size_a = args.embedding_input_size
            model_config.embedding_input_size_b = args.embedding_input_size
            model_config.embedding_input_size_c = args.embedding_input_size
            args.embedding_input_size_a = args.embedding_input_size
            args.embedding_input_size_b = args.embedding_input_size
            args.embedding_input_size_c = args.embedding_input_size
    else:
    ...
    elif args.model_type in ["lucapair1"]:
        model_class = LucaPair1
    elif args.model_type in ["lucapair2"]:
        model_class = LucaPair2
    elif args.model_type in ["lucapair3"]:
        model_class = LucaPair3
    elif args.model_type in ["lucapair4"]:
        model_class = LucaPair4
    elif args.model_type in ["lucapair5"]:
        model_class = LucaPair5
    elif args.model_type in ["lucapair6"]:
        model_class = LucaPair6
    elif args.model_type in ["lucatriple1"]:
        model_class = LucaTriple1
    elif args.model_type in ["lucatriple2"]:
        model_class = LucaTriple2
    elif args.model_type in ["lucaintrainter"]:
        model_class = LucaIntraInter
    elif args.model_type in ["lucaintraintertriple"]:
        model_class = LucaIntraInterTriple
    elif args.model_type == "luca_base":
        model_class = LucaBase
    else:
        raise Exception("Not support the model_type=%s" % args.model_type)

    if args.model_type in ["lucapair3", "lucapair4"]:
        model_config.decoder_layers = args.num_hidden_layers
        model_config.decoder_attention_heads = args.num_attention_heads
        if not hasattr(model_config, "decoder_ffn_dim") or model_config.decoder_ffn_dim is None or model_config.decoder_ffn_dim <= 0:
            model_config.decoder_ffn_dim = args.intermediate_size
    ...
    elif args.model_type in ["lucapair5", "lucapair6"]:
        model_config.encoder_layers = args.num_hidden_layers
        model_config.encoder_attention_heads = args.num_attention_heads
...
def main():
    ...
    if "luca_separated" in args.llm_type:
    ...
    elif "lucavirus" in args.llm_type:
        llm_dirpath = "%s/llm/models/lucavirus/%s/%s/%s/%s/checkpoint-%s" % (
            args.llm_dir if args.llm_dir else "..",
            args.llm_version,
            args.llm_task_level,
            args.llm_type,
        ...
        if not os.path.exists(llm_dirpath):
            llm_dirpath = "%s/llm/models/lucavirus/%s/%s/%s/%s/checkpoint-step%s" % (
                args.llm_dir if args.llm_dir else "..",
                args.llm_version,
                args.llm_task_level,
                args.llm_type,
            ...
            llm_dirpath = "%s/llm/models/lucagplm/%s/%s/%s/%s/checkpoint-%s" % (
                ...
                args.llm_version,
                args.llm_task_level,
                "lucaone_gplm" if args.llm_type == "lucaone" else args.llm_type,
                args.llm_time_str,
                args.llm_step
            )
            if not os.path.exists(llm_dirpath):
    ...
    if args.input_mode in ["pair"]:
        ...
        encoder_config = {
            ...
            "llm_task_level": args.llm_task_level,
        ...
        assert args.seq_max_length is not None or (args.seq_max_length_a is not None and args.seq_max_length_b is not None and args.seq_max_length_c is not None)
        if args.seq_max_length is None:
            args.seq_max_length = max(args.seq_max_length_a, args.seq_max_length_b, args.seq_max_length_c)
        # encoder_config
        encoder_config = {
            "llm_dirpath": llm_dirpath,
            "llm_time_str": args.llm_time_str,
    ...
    encoder_config["save_emb_to_disk"] = not args.not_save_emb_to_disk
    encoder_config["fp16_embedding"] = args.fp16_embedding

    # file row parser
    # 文件记录解析函数
    encoder = Encoder(**encoder_config)
    # pair对数据集
    if "pair" in args.model_type or args.input_mode in ["pair"]:
        parse_row_func = encoder.encode_pair
    elif "triple" in args.model_type or args.input_mode in ["triple"]:
        # triple对数据集
        parse_row_func = encoder.encode_triple
    else:
        # 基因或者蛋白单记录数据集
    ...
    if args.local_rank in [0, -1]:
        ...
        if len(model_config.id2label) > 10:
            ...
            str_config = copy.deepcopy(model_config)
        # model_structure = []
        # print_model_structure(model, model_structure=model_structure, depth=0)
        ...
        if args.input_mode == "pair":
            ...
            train_dataset = train_dataset.map(
                lambda x: parse_row_func(
                    ...
                    x["seq_type_a"] if "seq_type_a" in x else "prot",
                    x["seq_type_b"] if "seq_type_b" in x else "prot",
                    x["seq_type_c"] if "seq_type_c" in x else "prot",
                    x["seq_a"],
                    x["seq_b"],
                    x["seq_c"],
                    x["vector_filename_a"] if "vector_filename_a" in x else None,
                    x["vector_filename_b"] if "vector_filename_b" in x else None,
                    x["vector_filename_c"] if "vector_filename_c" in x else None,
                    x["matrix_filename_a"] if "matrix_filename_a" in x else None,
                    x["matrix_filename_b"] if "matrix_filename_b" in x else None,
                    x["matrix_filename_c"] if "matrix_filename_c" in x else None,
                    x["label"] if "label" in x else None,
                ),
                batched=False
            )
