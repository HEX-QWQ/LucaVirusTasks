...
@file: encoder
@desc: encoder for LucaVirusTasks
'''
import os
import torch
import sys
...
try:
    ...
    from .llm.lucaone.get_embedding import predict_embedding as predict_embedding_luca
    from .llm.lucavirus.get_embedding import predict_embedding as predict_embedding_lucavirus
...
class Encoder(object):
    def __init__(
            ...
            save_emb_to_disk=True,
            fp16_embedding=False,
            **kwargs
    ):
        print("-" * 25 + "Encoder" + "-" * 25)
        ...
        self.save_emb_to_disk = save_emb_to_disk
        self.fp16_embedding = fp16_embedding
        # vector
        ...
        print("Encoder: matrix_dirpath=%s" % self.matrix_dirpath)
        # special tokens
    ...
    def __get_embedding__(self, seq_id, seq_type, seq, embedding_type):
            ...
            try:
                ...
                for dirpath in dirpath_list:
                    ...
                    if os.path.exists(emb_filepath):
                        ...
                        if "multi_" in seq_type and not isinstance(embedding_info, list):
                            embedding_info = [embedding_info]
                        self.put_into_buffer(seq_id, embedding_info)
                        return embedding_info
            except Exception as e:
                print("seq_id=%s, emb_filename=%s" % (seq_id, emb_filename))
                print(e)
                embedding_info = None
        ...
        if embedding_info is None:
            ...
            if "onehot" in self.llm_type:
                ...
                # to do
                pass
            elif "lucavirus" in self.llm_type or "lucaone_virus" in self.llm_type:
                if seq_type in ["multi_gene", "multi_prot"]:
                    ...
                    for cur_seq in cur_seqs:
                        ...
                        while True:
                            ...
                            if self.embedding_fixed_len_a_time and self.embedding_fixed_len_a_time > 0:
                                cur_embedding_info, cur_processed_seq = predict_embedding_lucavirus(
                                    self.llm_dirpath,
                                ...
                                if cur_embedding_info is not None and hasattr(self, "embedding_complete") \
                                    ...
                                    cur_embedding_info = complete_embedding_matrix(
                                        ...
                                        self.device,
                                        use_cpu=use_cpu,
                                        predict_embedding_func=predict_embedding_lucavirus,
                                        fp16=self.fp16_embedding
                                    )
                            else:
                                cur_embedding_info, cur_processed_seq = predict_embedding_lucavirus(
                    ...
                    while True:
                        # 设置了一次性推理长度
                        if self.embedding_fixed_len_a_time and self.embedding_fixed_len_a_time > 0:
                            embedding_info, processed_seq = predict_embedding_lucavirus(
                                self.llm_dirpath,
                                [seq_id, seq_type, seq],
                                self.trunc_type,
                                embedding_type,
    ...
    def encode_triple(
        ...
        seq_type_a = seq_type_a.strip().lower()
        seq_type_b = seq_type_b.strip().lower()
        seq_type_c = seq_type_c.strip().lower()
        # for embedding vector
        vector_a, vector_b, vector_c = None, None, None
        if self.input_type in ["vector", "seq_vector"]:
            if vector_filename_a is None:
                if seq_a is None:
                    raise Exception("seq_a is none and vector_filename_a is none")
            ...
            if vector_filename_c is None:
                if seq_c is None:
                    raise Exception("seq_c is none and vector_filename_c is none")
                elif seq_type_c == "molecule":
                    raise Exception("now not support embedding of the seq_type_c=%s" % seq_type_c)
                else:
                    vector_c = self.__get_embedding__(seq_id_c, seq_type_c, seq_c, "vector")
            elif isinstance(vector_filename_c, str):
                for vector_dir in self.vector_dirpath:
                    vector_filepath_c = os.path.join(vector_dir, vector_filename_c)
                    if os.path.exists(vector_filepath_c):
                        vector_c = torch.load(vector_filepath_c)
                        break
            elif isinstance(vector_filename_c, np.ndarray):
                vector_c = vector_filename_c
            else:
                raise Exception("vector_c is not filepath-str and np.ndarray")

        # for embedding matrix
        matrix_a, matrix_b, matrix_c = None, None, None
        if self.input_type in ["matrix", "seq_matrix"]:
            if matrix_filename_a is None:
                if seq_a is None:
                    raise Exception("seq_a is none and matrix_filename_a is none")
            ...
            if matrix_filename_c is None:
                if seq_c is None:
                    raise Exception("seq_c is none and matrix_filename_c is none")
                else:
                    matrix_c = self.__get_embedding__(seq_id_c, seq_type_c, seq_c, "matrix")
            elif isinstance(matrix_filename_c, str):
                for matrix_dir in self.matrix_dirpath:
                    matrix_filepath_c = os.path.join(matrix_dir, matrix_filename_c)
                    if os.path.exists(matrix_filepath_c):
                        matrix_c = torch.load(matrix_filepath_c)
                        break
            elif isinstance(matrix_filename_c, np.ndarray):
                matrix_c = matrix_filename_c
            else:
                raise Exception("matrix_c is not filepath-str and np.ndarray")
        ...
        if self.input_type in ["matrix", "seq_matrix"] and "esm" in self.llm_type:
            ...
            if seq_type_c == "prot":
                seq_c = clean_seq(seq_id_c, seq_c)
            elif seq_type_c == "multi_prot":
                seq_c = ",".join([clean_seq(seq_id_c, v) for v in seq_c.split(",")])
