...
@file: LucaTriple2
@desc: LucaTriple2 for heterogeneous three sequences
'''
...
class LucaTriple2(BertPreTrainedModel):
    def __init__(self, config, args):
        super(LucaTriple2, self).__init__(config)
        ...
        self.seq_encoder_c, self.seq_pooler_c, self.matrix_encoder_c, self.matrix_pooler_c = None, None, None, None
        ...
        self.encoder_type_list_c = [False, False, False]
        self.input_size_list_c = [0, 0, 0]
        self.linear_idx_c = [-1, -1, -1]
        if self.input_type == "seq":
            ...
            self.seq_encoder_b = BertModel(
                config,
                use_pretrained_embedding=False,
                add_pooling_layer=(args.seq_pooling_type is None or args.seq_pooling_type == "none") and self.task_level_type in ["seq_level"]
            )
            self.input_size_list_c[0] = config.hidden_size
            config.max_position_embeddings = config.seq_max_length_c
            self.seq_encoder_c = BertModel(
                config,
                use_pretrained_embedding=False,
                add_pooling_layer=(args.seq_pooling_type is None or args.seq_pooling_type == "none") and self.task_level_type in ["seq_level"]
            )
            if self.task_level_type in ["seq_level"]:
                ...
                self.seq_pooler_c = create_pooler(pooler_type="seq", config=config, args=args)
            self.encoder_type_list_a[0] = True
            self.encoder_type_list_b[0] = True
            self.encoder_type_list_c[0] = True
            self.linear_idx_a[0] = 0
            self.linear_idx_b[0] = 0
            self.linear_idx_c[0] = 0
            ...
            if args.matrix_encoder:
                ...
                matrix_encoder_config_c = copy.deepcopy(config)
                matrix_encoder_config_c.no_position_embeddings = True
                matrix_encoder_config_c.no_token_type_embeddings = True
                matrix_encoder_config_c.embedding_input_size = config.embedding_input_size_c
                matrix_encoder_config_c.max_position_embeddings = config.matrix_max_length_c
                if args.matrix_encoder_act:
                    ...
                    self.matrix_encoder_c = nn.ModuleList([
                        nn.Linear(config.embedding_input_size_c, config.hidden_size),
                        create_activate(config.emb_activate_func),
                        # nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps),
                        BertModel(
                            matrix_encoder_config_c,
                            use_pretrained_embedding=True,
                            add_pooling_layer=(args.matrix_pooling_type is None or args.matrix_pooling_type == "none") and self.task_level_type in ["seq_level"])
                    ])
                else:
                ...
                if self.task_level_type in ["seq_level"]:
                    ...
                    self.matrix_pooler_c = create_pooler(pooler_type="matrix", config=config, args=args)
                self.input_size_list_a[1] = config.embedding_input_size
                self.input_size_list_b[1] = config.embedding_input_size
                self.input_size_list_c[1] = config.embedding_input_size
                ...
                self.input_size_list_c[1] = config.embedding_input_size_c
                if self.task_level_type in ["seq_level"]:
                    ori_embedding_input_size = config.embedding_input_size
                    ...
                    self.matrix_pooler_b = create_pooler(pooler_type="matrix", config=config, args=args)
                    config.embedding_input_size = config.embedding_input_size_c
                    self.matrix_pooler_c = create_pooler(pooler_type="matrix", config=config, args=args)
                    config.embedding_input_size = ori_embedding_input_size
            self.encoder_type_list_a[1] = True
            self.encoder_type_list_b[1] = True
            self.encoder_type_list_c[1] = True
            self.linear_idx_a[1] = 0
            self.linear_idx_b[1] = 0
            self.linear_idx_c[1] = 0
            ...
            self.input_size_list_c[2] = config.embedding_input_size_c
            self.encoder_type_list_c[2] = True
            self.linear_idx_c[2] = 0
            ...
            if args.matrix_encoder:
                ...
                matrix_encoder_config_c.no_token_type_embeddings = True
                matrix_encoder_config_c.max_position_embeddings = config.matrix_max_length_c
                if args.matrix_encoder_act and (
                        config.embedding_input_size_a != config.embedding_input_size_b
                        or config.embedding_input_size_a != config.embedding_input_size_c
                        or config.embedding_input_size_b != config.embedding_input_size_c
                ):
                    self.matrix_encoder_a = nn.ModuleList([
                        nn.Linear(config.embedding_input_size_a, config.hidden_size),
                        create_activate(config.emb_activate_func),
                ...
                self.input_size_list_c[1] = config.hidden_size
                config.embedding_input_size = ori_embedding_input_size
            else:
                self.input_size_list_a[1] = config.embedding_input_size_a
                self.input_size_list_b[1] = config.embedding_input_size_b
        ...
        for encoder_idx, encoder_flag in enumerate(self.encoder_type_list_a):
            ...
            input_size_c = self.input_size_list_c[encoder_idx]
            print("encoder_idx:", encoder_idx, ", input_size_a:", input_size_a, ", input_size_b:", input_size_b, ", input_size_c:", input_size_c)
        ...
        all_linear_list_c = [linear for linear in all_linear_list_c if linear is not None]
        ...
        if all_linear_list_c is not None and len(all_linear_list_c) > 0:
            self.linear_c = nn.ModuleList(all_linear_list_c)
        if self.fusion_type == "add":
            ...
            output_size_c = [v for v in self.output_size_c if v > 0]
            assert len(set(output_size_c)) == 1
            last_hidden_size_c = output_size_c[0]
        else:
            last_hidden_size_a = sum(self.output_size_a)
            last_hidden_size_b = sum(self.output_size_b)
            last_hidden_size_c = sum(self.output_size_c)
        last_hidden_size = last_hidden_size_a + last_hidden_size_b + last_hidden_size_c
    ...
    def __forward_c__(
        ...
        if input_ids is not None and self.seq_encoder_c is not None:
            seq_outputs = self.seq_encoder_c(
                input_ids,
                attention_mask=seq_attention_masks,
                token_type_ids=token_type_ids,
                position_ids=position_ids,
            ...
            if self.seq_pooler_c is not None:
                seq_vector = self.seq_pooler_c(seq_outputs[0], mask=seq_attention_masks)
            elif self.task_level_type in ["seq_level"]:
                seq_vector = seq_outputs[1]
            else:
                seq_vector = seq_outputs[0]
            seq_linear_idx = self.linear_idx_c[0]
            if seq_linear_idx != -1:
                for i, layer_module in enumerate(self.linear_c[seq_linear_idx]):
                    seq_vector = layer_module(seq_vector)
        if vectors is not None:
            vector_vector = vectors
            vector_linear_idx = self.linear_idx_c[2]
            if vector_linear_idx != -1:
                for i, layer_module in enumerate(self.linear_c[vector_linear_idx]):
                    vector_vector = layer_module(vector_vector)
        if matrices is not None:
            if self.matrix_encoder_c is not None:
                for module in self.matrix_encoder_c[:-1]:
                    matrices = module(matrices)
                matrices_output = self.matrix_encoder_c[-1](
                    input_ids=None,
                    attention_mask=matrix_attention_masks,
                    token_type_ids=None,
                    position_ids=None,
            ...
            if self.matrix_pooler_c is not None:
                matrix_vector = self.matrix_pooler_c(matrices, mask=matrix_attention_masks)
            elif self.task_level_type in ["seq_level"]:
                tmp_mask = torch.unsqueeze(matrix_attention_masks, dim=-1)
                matrices = matrices.masked_fill(tmp_mask == 0, 0.0)
                # 均值pooling
                ...
                matrix_vector = matrices
            matrix_linear_idx = self.linear_idx_c[1]
            if matrix_linear_idx != -1:
                for i, layer_module in enumerate(self.linear_c[matrix_linear_idx]):
                    matrix_vector = layer_module(matrix_vector)
    ...
    def forward(
            self,
            input_ids_a=None, input_ids_b=None, input_ids_c=None,
            position_ids_a=None, position_ids_b=None, position_ids_c=None,
            token_type_ids_a=None, token_type_ids_b=None, token_type_ids_c=None,
            seq_attention_masks_a=None, seq_attention_masks_b=None, seq_attention_masks_c=None,
            vectors_a=None, vectors_b=None, vectors_c=None,
            matrices_a=None, matrices_b=None, matrices_c=None,
            matrix_attention_masks_a=None, matrix_attention_masks_b=None, matrix_attention_masks_c=None,
            labels=None,
            **kwargs
    ):
        ...
        representation_vector_c = self.__forward_c__(
            input_ids_c,
        ...
        concat_vector = torch.cat([representation_vector_a, representation_vector_b, representation_vector_c], dim=-1)
