...
class LucaDecoderLayer(nn.Module):
    """
    Decoder Layer
    """
    def __init__(self, config: PretrainedConfig):
        ...
        if self.use_moe:
            print("LucaDecoderLayer use MoE!")
        ...
        if config.attn_type == "gqa":
            self.self_attn = LucaGroupedQueryAttention(
                ...
                n_kv_heads=config.decoder_kv_attention_heads,
        ...
        if config.attn_type == "gqa":
            self.encoder_attn = LucaGroupedQueryAttention(dim=self.embed_dim,
                                                          n_heads=config.decoder_attention_heads,
                                                          n_kv_heads=config.decoder_kv_attention_heads,
                                                          max_batch_size=config.max_batch_size,
                                                          max_seq_len=config.max_seq_len
            ...
            self.encoder_attn = Luca_ATTENTION_CLASSES[config.attn_type](
                self.embed_dim,
        ...
        # cross
        if config.layer_norm_name == "RMSNorm":
            self.encoder_attn_layer_norm = LucaRMSNorm(dim=self.embed_dim, eps=config.layer_norm_eps)
        elif config.layer_norm_name == "FlashRMSNorm":
            self.encoder_attn_layer_norm = LucaFlashRMSNorm(dim=self.embed_dim, eps=config.layer_norm_eps)
        else:
...
class LucaDecoder(PreTrainedModel):
    """
    Decoder
    """
    def __init__(self,
        ...
        self.dropout = config.dropout
        self.layer_dropout = config.decoder_layer_dropout
        self.max_target_positions = config.max_position_embeddings
        self.use_rotary_position_embeddings = config.use_rotary_position_embeddings
        self.use_token_type_embeddings = config.use_token_type_embeddings
        self.layer_norm_type = config.layer_norm_type
        self.embed_dim = config.embed_dim if hasattr(config, "embed_dim") else config.hidden_size
        ...
        self.layers = nn.ModuleList([LucaDecoderLayer(config) for _ in range(config.decoder_layers)])
