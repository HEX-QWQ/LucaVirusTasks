...
class LucaTransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        :param layer_norm_type: the layer norm type('post' or 'pre')
    ...
    def _init_submodules(self):
        ...
        if self.layer_norm_type == "pre":
            # pre layer norm
            self.pre_layer_norm = luca_layer_norm(self.embed_dim, eps=self.layer_norm_eps)
        ...
        if self.layer_norm_type != "pre":
            self.self_attn_layer_norm = luca_layer_norm(self.embed_dim, eps=self.layer_norm_eps)
        else:
            # post layer norm
            self.post_layer_norm = luca_layer_norm(self.embed_dim, eps=self.layer_norm_eps)
        ...
        # post
        if self.layer_norm_type != "pre":
            self.final_layer_norm = luca_layer_norm(self.embed_dim, eps=self.layer_norm_eps)
...
class LucaTransformer(nn.Module):
    def __init__(self, config, emb_layer=None, use_pretrained_embedding=False, add_pooling_layer=True):
        ...
        if hasattr(config, "use_luca_layer_norm_v2"):
            ...
            luca_layer_norm = LucaLayerNormV2
        else:
            luca_layer_norm = LucaLayerNormV1
        ...
        if self.layer_norm_type == "pre":
            ...
            self.embed_layer_norm = luca_layer_norm(self.embed_dim, eps=self.layer_norm_eps)
        ...
        self.encoder = nn.ModuleList(
            [
                LucaTransformerLayer(
                    ...
                    add_bias_kv=config.add_bias_kv if hasattr(config, "add_bias_kv") else False,
        ...
        if self.layer_norm_type == "pre":
            self.last_layer_norm = luca_layer_norm(self.config.hidden_size, eps=self.layer_norm_eps)
...
class LucaCrossTransformer(nn.Module):
    def __init__(self, config, emb_layer_a=None, emb_layer_b=None, shared=False, use_pretrained_embedding=False, add_pooling_layer=True):
        ...
        if self.layer_norm_type == "pre":
            self.embed_layer_norm_a, self.embed_layer_norm_b = None, None
        else:
            self.embed_layer_norm_a, self.embed_layer_norm_b = luca_layer_norm(self.embed_dim, eps=self.layer_norm_eps),\
                                                               luca_layer_norm(self.embed_dim, eps=self.layer_norm_eps)
        ...
        self.layer_size = self.config.num_hidden_layers
        if self.layer_norm_type == "pre":
            self.last_layer_norm_a = luca_layer_norm(self.config.hidden_size, eps=self.layer_norm_eps)
            if self.shared:
                self.last_layer_norm_b = self.last_layer_norm_a
            else:
                self.last_layer_norm_b = luca_layer_norm(self.config.hidden_size, eps=self.layer_norm_eps)
    ...
    def forward(
        ...
        if self.layer_norm_type != "pre":
            x_a = self.embed_layer_norm(x_a)
            x_b = self.embed_layer_norm(x_b)
