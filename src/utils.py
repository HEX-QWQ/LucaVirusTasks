...
# 自定义递归打印函数
def print_model_structure(module, model_structure, depth=0):
    children = list(module.children())
    if not children:
        return
    else:
        for child in children:
            model_structure.append(" " * depth * 2 + str(child))
            print_model_structure(child, model_structure, depth + 1)
...
def download_trained_checkpoint_lucaone(
    ...
    try:
        ...
        logs_path = "logs/lucagplm/%s/%s/%s/%s" % (
            llm_version,
            llm_task_level,
            "lucaone_gplm" if llm_type == "lucaone" else llm_type,
            llm_time_str
        )
...
def download_trained_checkpoint_lucavirus(
        llm_dir,
        llm_type="lucavirus",
        llm_version="v1.0",
...
def download_trained_checkpoint_downstream_tasks(
    ...
    for idx in range(len(dataset_name)):
        try:
            ...
            models_file_names = ["config.json", "model.safetensors", "training_args.bin", "tokenizer/alphabet.pkl"]
