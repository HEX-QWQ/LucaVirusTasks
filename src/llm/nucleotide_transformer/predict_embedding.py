...
@desc: embedding inference for Nucleotide Transformer
'''
import os
import sys
import torch
...
def predict_embedding(
    ...
    if torch.cuda.is_available() and device is not None:
        nucl_transformer_global_model = nucl_transformer_global_model.to(device)
    elif torch.cuda.is_available():
        nucl_transformer_global_model = nucl_transformer_global_model.cuda()
    ...
    '''
    if device is None:
        device = next(nucl_transformer_global_model.parameters()).device
    else:
        model_device = next(nucl_transformer_global_model.parameters()).device
        if device != model_device:
            nucl_transformer_global_model = nucl_transformer_global_model.to(device)
    nucl_transformer_global_model.eval()

    inputs = nucl_transformer_global_alphabet(processed_seq, return_tensors='pt')["input_ids"]
    embeddings = {}
    with torch.no_grad():
...
def get_args():
    parser = argparse.ArgumentParser(description='nucl_transformer Embedding')
    # for one seq
