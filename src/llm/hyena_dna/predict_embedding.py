...
@desc: embedding inference for HyenaDNA
'''
import os
import sys
import torch
        ...
        try:
            ...
            if "representations" in embedding_type or "matrix" in embedding_type:
                if matrix_add_special_token:
                    embedding = out[0, 0: truncate_len + 2].cpu().to(torch.float16).clone().numpy()
                else:
                    embedding = out[0, 1: truncate_len + 1].cpu().to(torch.float16).clone().numpy()
