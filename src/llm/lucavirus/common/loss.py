...
try:
    ...
    from src.llm.lucavirus.common.classification_loss import *
    from src.llm.lucavirus.common.regression_loss import *
from transformers.activations import NewGELUActivation, FastGELUActivation, QuickGELUActivation
...
def create_activate(activate_func):
    ...
    if activate_func == "tanh":
        ...
        return nn.ReLU()
    elif activate_func in ["leakyrelu", "leaky_relu"]:
        return nn.LeakyReLU()
    elif activate_func == "gelu":
        return nn.GELU()
    elif activate_func == "gelu_new":
        return NewGELUActivation()
    elif activate_func == "gelu_fast":
        return FastGELUActivation()
    elif activate_func == "quick_gelu":
    ...
    if "loss" in return_types:
        ...
        if output_mode in ["regression"]:
            ...
            if weight is not None:
                ...
                if loss_type == "soft_cce":
                    assert hasattr(args, "label_relations")
                    loss_fct = MaskedSoftCrossEntropyLoss(
                        label_relations=args.label_relations,
                        weight=weight,
                        reduction=reduction,
                        ignore_nans=True,
                        ignore_value=ignore_index
                    ...
                    if ignore_index is None:
                        ...
                        loss_fct = MaskedCrossEntropyLoss(weight=weight,
                                                          reduction=reduction,
                                                          ignore_nans=True,
                                                          ignore_value=ignore_index)
            else:
