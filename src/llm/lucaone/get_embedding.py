...
try:
    ...
    from utils import set_seed, to_device, get_labels, get_parameter_number, seq_type_is_match_seq, \
        gene_seq_replace, clean_seq_luca, available_gpu_id, download_trained_checkpoint_lucaone, \
    ...
    from src.utils import set_seed, to_device, get_labels, get_parameter_number, seq_type_is_match_seq, \
        gene_seq_replace, clean_seq_luca, available_gpu_id, download_trained_checkpoint_lucaone, \
        calc_emb_filename_by_seq_id
    from src.llm.lucaone.v0_2.lucaone_gplm import LucaGPLM as LucaGPLMV0_2
    from src.llm.lucaone.v0_2.lucaone_gplm_config import LucaGPLMConfig as LucaGPLMConfigV0_2
    from src.llm.lucaone.v0_2.alphabet import Alphabet as AlphabetV0_2
    from src.llm.lucaone.v2_0.lucaone_gplm import LucaGPLM as LucaGPLMV2_0
    from src.llm.lucaone.v2_0.lucaone_gplm_config import LucaGPLMConfig as LucaGPLMConfigV2_0
    from src.llm.lucaone.v2_0.alphabet import Alphabet as AlphabetV2_0
...
lucaone_global_tokenizer = None, None, None, None
...
def load_model(
    ...
    if len(strs) > 1:
        ...
        lucaone_checkpoint = "5600000"
        default = True
        for s_idx, s in enumerate(ss):
            if s.startswith("checkpoint-step"):
                lucaone_checkpoint = s.replace("checkpoint-step", "")
                lucaone_time_str = ss[s_idx - 1]
                lucaone_type = ss[s_idx - 2]
                lucaone_task_level = ss[s_idx - 3]
                lucaone_version = ss[s_idx - 4]
                default = False
                break
        if not default:
            download_trained_checkpoint_lucaone(
                llm_dir=os.path.join(strs[0], "llm/"),
                llm_type=lucaone_type,
                llm_task_level=lucaone_task_level,
                llm_version=lucaone_version,
                llm_time_str=lucaone_time_str,
                llm_step=lucaone_checkpoint
            )
        else:
            download_trained_checkpoint_lucaone(
                llm_dir=os.path.join(strs[0], "llm/"),
...
def get_args():
    ...
    parser.add_argument(
        "--seq_type", type=str, default=None,
        required=True,
    ...
    parser.add_argument(
        ...
        type=str,
        default="lucaone",
        choices=["lucaone_gplm", "lucaone",  "lucagplm"],
        help="the llm type"
    )
    parser.add_argument(
        "--llm_version",
...
def main(model_args):
    ...
    cur_log_filepath = "%s/llm/logs/lucagplm/%s/%s/%s/%s/logs.txt" % (
        ...
        "lucaone_gplm" if model_args.llm_type == "lucaone" else model_args.llm_type,
