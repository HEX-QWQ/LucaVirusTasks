        ...
        try:
            ...
            if self.input_mode == "pair":
                if len(row) == 5:
                    ...
                    vector_filename_a, vector_filename_b, matrix_filename_a, matrix_filename_b, label = row[0:11]
                else:
            ...
            elif self.input_mode == "triple":
                if len(row) == 10:
                    seq_id_a, seq_id_b, seq_id_c, seq_type_a, seq_type_b, seq_type_c, seq_a, seq_b, seq_c, label = row[0:10]
                    vector_filename_a, vector_filename_b, vector_filename_c, matrix_filename_a, matrix_filename_b, matrix_filename_c = None, None, None, None, None, None
                elif len(row) == 16:
                    seq_id_a, seq_id_b, seq_id_c, seq_type_a, seq_type_b, seq_type_c, seq_a, seq_b, seq_c, \
                    vector_filename_a, vector_filename_b, vector_filename_c, matrix_filename_a, matrix_filename_b, matrix_filename_c, \
                    label = row[0:16]
                else:
                    raise Exception("the cols num not in [10, 16]")
                res = {
                    "seq_id_a": seq_id_a,
                    "seq_id_b": seq_id_b,
                    "seq_id_c": seq_id_c,
                ...
                if self.input_type in ["vector", "seq_vector"]:
                    res.update({
                        "vector_filename_a": vector_filename_a,
                        "vector_filename_b": vector_filename_b,
                        "vector_filename_c": vector_filename_c
                    })
                if self.input_type in ["matrix", "seq_matrix"]:
                    res.update({
                        "matrix_filename_a": matrix_filename_a,
                        "matrix_filename_b": matrix_filename_b,
                        "matrix_filename_c": matrix_filename_c
                    })
            else:
                if len(row) == 3:
