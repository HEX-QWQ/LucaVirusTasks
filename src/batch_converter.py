...
@desc: batch converter for LucaVirusTasks
'''
import sys
import torch
...
class BatchConverter(object):
    ...
    def __multi_seq_encode__(self, batch_size, seqs):
        ...
        if self.seq_subword:
            ...
            seq_encoded_list = []
            for cur_sample_seqs in seqs:
                # print("cur_sample_seqs:", len(cur_sample_seqs))
                cur_seq_encoded_list = []
                if len(cur_sample_seqs) > self.max_sentences:
                    # 每个样本最多cur_sample_seqs个
                    if self.trunc_type == "left":
                ...
                seq_encoded_list.append(cur_seq_encoded_list)
            # print("seq_encoded_list: %d" % len(seq_encoded_list))
        # for input
        input_ids = torch.empty(
            (
                batch_size,
        ...
        return seq_encoded_list, input_ids, position_ids, token_type_ids, \
               attention_masks, max_sentence_num, max_sentence_len
    ...
    def __call_single__(self, batch_size, seq_types, seqs, vectors, matrices, labels):
        ...
        if seqs:
            ...
            if molecule_flag:
                ...
                seq_encoded_list, input_ids, position_ids, token_type_ids, seq_attention_masks, seq_max_num, seq_max_len \
                    = self.__multi_seq_encode__(
        ...
        """
        print("multi_seq_flag: %r" % multi_seq_flag)
        print("new_seqs: %d" % len(new_seqs))
        print("seq_encoded_list: %d" % len(seq_encoded_list))
        input("continue:")
        """
    ...
    def __call__(self, raw_batch: Sequence[dict]):
        ...
        if "seq_id_a" in raw_batch[0] and "seq_id_b" in raw_batch[0] and "seq_id_c" in raw_batch[0]:
            # triple
            res = {}
            # seq_ids_a = []
            seq_types_a = []
            ...
            for item in raw_batch:
                ...
                # seq_ids_c.append(item["seq_id_c"])
                seq_types_c.append(item["seq_type_c"])
                if item["seq_c"] is not None:
                    seqs_c.append(item["seq_c"])
                if item["vector_c"] is not None:
                    vectors_c.append(item["vector_c"])
                if item["matrix_c"] is not None:
                    matrices_c.append(item["matrix_c"])

                if "label" in item and item["label"] is not None:
                    labels.append(item["label"])
            # for input_a
            input_ids_a, position_ids_a, token_type_ids_a, seq_attention_masks_a, \
            encoded_vectors_a, encoded_matrices_a, matrix_attention_masks_a, num_sentences_a, sentence_length_a, labels \
            ...
            # for input_b
            input_ids_b, position_ids_b, token_type_ids_b, seq_attention_masks_b, encoded_vectors_b, \
            encoded_matrices_b, matrix_attention_masks_b, num_sentences_b, sentence_length_b,  _ \
            ...
            # for input_c
            input_ids_c, position_ids_c, token_type_ids_c, seq_attention_masks_c, encoded_vectors_c, \
            encoded_matrices_c, matrix_attention_masks_c, num_sentences_c, sentence_length_c,  _ \
                = self.__call_single__(batch_size, seq_types_c, seqs_c, vectors_c, matrices_c, labels=None)
            if not hasattr(self, "max_sentences") or self.max_sentences is None:
                res.update({
                    "input_ids_c": input_ids_c,
                    "position_ids_c": position_ids_c,
                    "token_type_ids_c": token_type_ids_c,
                    "seq_attention_masks_c": seq_attention_masks_c,
                    "vectors_c": encoded_vectors_c,
                    "matrices_c": encoded_matrices_c,
                    "matrix_attention_masks_c": matrix_attention_masks_c
                })
            else:
                res.update({
                    "input_ids_c": input_ids_c,
                    ...
                    "vectors_c": encoded_vectors_c,
                    "matrices_c": encoded_matrices_c,
                    "num_sentences_c": num_sentences_c,
                    "sentence_length_c": sentence_length_c,
                    "matrix_attention_masks_c": matrix_attention_masks_c
                })
            return res
        elif "seq_id_a" in raw_batch[0] and "seq_id_b" in raw_batch[0]:
            # pair
            res = {}
            # seq_ids_a = []
            seq_types_a = []
