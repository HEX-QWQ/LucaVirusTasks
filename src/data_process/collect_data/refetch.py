...
def cds_record_generator(row, fetch_data,err_file,refetch_list):
    """
    ...
    for accession in accession_list:
        if accession not in refetch_list:
            continue
        print(f'try to refetch {accession}')
        try:
            genbank = fetch_data.fetch_genbank(accession,err_file=err_file)
            if not genbank:
                # 返回空列表，也记录到错误文件
...
def main():
    ...
    data_dir = Path("./data")
    # 读取整个 Excel（小文件可行）
    df = pd.read_excel(input_file, sheet_name="VMR_MSL40")
    df.columns = df.columns.str.strip().str.replace(r"\s+", "_", regex=True)
    refetch_list = ['MH540083','HQ337022','KP143762','OP072690',
                'MF158038','ON645338','MH271301','EU432373',
                'CASDWV010003587','KM213865','BK061758','MK503255',
                'JX417822','AJ132269','AM260465','MW355479']
    # refetch_list = ['DNA-A: EU432373','MK503255','JX417822']
    ...
    with open(err_file_path, "w", encoding="utf-8") as err_file:
        ...
        for _, row in tqdm(df.iterrows(), total=len(df), desc="Processing rows"):
            for record in cds_record_generator(row, fetch_data,err_file,refetch_list=refetch_list):
