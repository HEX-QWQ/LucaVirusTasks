...
try:
    ...
    from common.luca_encoder import LucaEncoder
    ...
    from src.common.luca_encoder import LucaEncoder
    from src.common.modeling_bert import BertModel, BertPreTrainedModel
logger = logging.getLogger(__name__)


class LucaPair6(BertPreTrainedModel):
    def __init__(self, config, args):
        super(LucaPair6, self).__init__(config)
        ...
        if config.hidden_size != config.embedding_input_size_b:
            ...
            self.linear_b = None
        self.encoder_a = LucaEncoder(config)
        self.encoder_b = LucaEncoder(config)
        config.embedding_input_size = config.hidden_size
        self.pooler_a = create_pooler(pooler_type="matrix", config=config, args=args)
        self.pooler_b = create_pooler(pooler_type="matrix", config=config, args=args)
        self.dropout, self.hidden_layer, self.hidden_act, self.classifier, self.output, self.loss_fct = \
            create_loss_function(
                config,
                args,
                hidden_size=config.hidden_size + config.hidden_size if self.fusion_type == "concat" else config.hidden_size,
    ...
    def forward(
        ...
        #  encoder+对称结构 matrices_b作为左边，matrices_a作为右边 + matrices_a作为左边，matrices_b作为右边
        ...
        last_hidden_states_a = self.encoder_a(
            hidden_states=hidden_states_a,
            attention_mask=matrix_attention_masks_a,
            cross_hidden_states=hidden_states_b,
            cross_attention_mask=matrix_attention_masks_b,
            output_attentions=output_attentions,
            use_cache=False,
            return_dict=True
        ).last_hidden_state
        last_hidden_states_b = self.encoder_b(
            hidden_states=hidden_states_b,
            attention_mask=matrix_attention_masks_b,
            cross_hidden_states=hidden_states_a,
            cross_attention_mask=matrix_attention_masks_a,
            output_attentions=output_attentions,
            use_cache=False,
            return_dict=True
        ).last_hidden_state

        if self.pooler_a is not None:
            last_hidden_states_a = self.pooler_a(last_hidden_states_a, mask=matrix_attention_masks_a)
        if self.pooler_b is not None:
            last_hidden_states_b = self.pooler_b(last_hidden_states_b, mask=matrix_attention_masks_b)

        if self.dropout is not None:
            last_hidden_states_b = self.dropout(last_hidden_states_b)
            last_hidden_states_a = self.dropout(last_hidden_states_a)

        if self.fusion_type == "add":
            last_hidden_states = torch.add(last_hidden_states_a, last_hidden_states_b)
        else:
            last_hidden_states = torch.cat([last_hidden_states_a, last_hidden_states_b], dim=-1)
