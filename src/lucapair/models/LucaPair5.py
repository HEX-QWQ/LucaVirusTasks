...
class LucaPair5(BertPreTrainedModel):
    def __init__(self, config, args):
        super(LucaPair5, self).__init__(config)
        ...
        if config.hidden_size != config.embedding_input_size_b:
            ...
            self.linear_b = None
        self.encoder = LucaEncoder(config)
    ...
    def forward(
        ...
        # encoder非对称结构：matrices_a作为左边，matrices_b作为右边
        ...
        if self.linear_a is not None:
            ...
            cross_hidden_states = self.linear_a(matrices_a)
        else:
            cross_hidden_states = matrices_a
