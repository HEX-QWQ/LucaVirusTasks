...
class LucaPair4(BertPreTrainedModel):
    def __init__(self, config, args):
        super(LucaPair4, self).__init__(config)
        ...
        if config.hidden_size != config.embedding_input_size_b:
            ...
            self.linear_b = None
        self.decoder_a = LucaDecoder(config)
        self.decoder_b = LucaDecoder(config)
    ...
    def forward(
        ...
        # decoder对称结构：matrices_a作为右边，matrices_b作为左边 + matrices_a作为左边，matrices_b作为右边
        ...
        last_hidden_states_a = self.decoder_a(
            hidden_states=hidden_states_a,
            attention_mask=matrix_attention_masks_a,
            encoder_hidden_states=hidden_states_b,
            encoder_attention_mask=matrix_attention_masks_b,
            output_attentions=output_attentions,
            use_cache=False,
            return_dict=True
        ).last_hidden_state
        last_hidden_states_b = self.decoder_b(
            hidden_states=hidden_states_b,
            attention_mask=matrix_attention_masks_b,
            encoder_hidden_states=hidden_states_a,
